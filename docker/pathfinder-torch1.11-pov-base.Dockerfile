FROM pathfinder-torch1.11-base
...
RUN cd $PYTORCH && \
    CC=clang-13 CXX=clang++-13 CMAKE_PREFIX_PATH=$CONDA_HOME CMAKE_C_COMPILER=clang-13 CMAKE_CXX_COMPILER=clang++-13 DEBUG=1 BUILD_TEST=0 USE_MKLDNN=0 USE_OPENMP=0 USE_GLOO=0 USE_BREAKPAD=0 USE_FBGEMM=0 USE_CUDA=0 USE_NCCL=0 $CONDA_RUN python3 setup.py install && \
    rm -rf $PYTORCH/build
