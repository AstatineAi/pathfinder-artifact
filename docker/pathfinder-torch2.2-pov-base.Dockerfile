...
RUN cd $PYTORCH && \
    CC=clang CXX=clang++ CMAKE_PREFIX_PATH=$CONDA_HOME CMAKE_C_COMPILER=clang CMAKE_CXX_COMPILER=clang++ DEBUG=1 BUILD_TEST=0 USE_MKLDNN=0 USE_OPENMP=0 USE_CUDA=0 USE_NCCL=0 $CONDA_RUN python3 setup.py install && \
    rm -rf $PYTORCH/build
